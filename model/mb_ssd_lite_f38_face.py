...
def create_mb_ssd_lite_f38_face(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=True,
    ...
    regression_headers = ModuleList([
        ...
        SeperableConv2d(in_channels=512, out_channels=anchors[3] * 4, kernel_size=3, padding=1,
                        onnx_compatible=onnx_compatible),
        SeperableConv2d(in_channels=256, out_channels=anchors[3] * 4, kernel_size=3, padding=1,
                        onnx_compatible=onnx_compatible),
        SeperableConv2d(in_channels=256, out_channels=anchors[4] * 4, kernel_size=3, padding=1,
                        onnx_compatible=onnx_compatible),
    ...
    landmarks_headers = ModuleList([
        ...
        SeperableConv2d(in_channels=512, out_channels=anchors[3] * 10, kernel_size=3, padding=1,
                        onnx_compatible=onnx_compatible),
        SeperableConv2d(in_channels=256, out_channels=anchors[3] * 10, kernel_size=3, padding=1,
                        onnx_compatible=onnx_compatible),
        SeperableConv2d(in_channels=256, out_channels=anchors[4] * 10, kernel_size=3, padding=1,
                        onnx_compatible=onnx_compatible),
        Conv2d(in_channels=64, out_channels=anchors[5] * 10, kernel_size=1),
    ])

    return SSD(num_classes, base_net, source_layer_indexes,
               extras, classification_headers, regression_headers, landmarks_headers, is_test=is_test, config=config)


def create_mb_ssd_lite_f38_person_predictor(net, candidate_size=200, nms_method=None, sigma=0.5,
