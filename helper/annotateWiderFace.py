...
def analyze(new_label, image_name, detections, ground_truth, iou_th=0.9):
    gt_to_detection = {}
    detection_to_gt = {}
    tp = 0
    tp_e2e = 0
    ...
    new_label.append('# ' + image_name)
    for i in range(0, len(ground_truth)):
        image_gt = ground_truth[i]
        gt_bbox = [image_gt[0], image_gt[1], image_gt[2], image_gt[3]]
        ...
        if gt_lands[0] > 0:
            new_label.append(image_gt)
            continue

        for dets in range(len(detections)):
            det_bbox = [dets[0], dets[1], dets[2], dets[3]]
            det_lands = [image_gt[4], image_gt[5], 0, image_gt[7], image_gt[8], 0, image_gt[10], image_gt[11], 0,
                         image_gt[13], image_gt[14], 0, image_gt[16], image_gt[17], 0, 0]
            ...
            if ratio > iou_th:
                new_label.append(gt_bbox + det_lands)
    return new_label
...
if __name__ == "__main__":
    ...
    for index, name in enumerate(images_name):
        ...
        analyze(new_label, name, predict, gt)
        print("test")
